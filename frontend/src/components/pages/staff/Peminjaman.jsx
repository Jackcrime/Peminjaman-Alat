...
const Peminjaman = () => {
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('Laporan Peminjaman Alat', 14, 22);
    
    doc.setFontSize(11);
    doc.text(`Tanggal: ${new Date().toLocaleDateString('id-ID')}`, 14, 30);
    doc.text(`Filter: ${filter || 'Semua'}`, 14, 36);
    ...
    autoTable(doc, {
      head: [['No', 'Peminjam', 'Alat', 'Jumlah', 'Tanggal', 'Status']],
      body: tableData,
      startY: 42,
      styles: { fontSize: 9 },
    ...
    doc.save(`peminjaman-${new Date().getTime()}.pdf`);
  };

  const exportToExcel = () => {
    const excelData = filteredData.map((item, index) => ({
      'No': index + 1,
      'Peminjam': item.user?.name || 'N/A',
      'Email': item.user?.email || 'N/A',
      'Alat': item.alat?.nama_alat || 'N/A',
      'Kode Alat': item.alat?.kode_alat || 'N/A',
      'Jumlah': item.jumlah || 0,
      'Tanggal Pinjam': item.tgl_peminjaman ? new Date(item.tgl_peminjaman).toLocaleDateString('id-ID') : '-',
      'Tanggal Kembali': item.tgl_pengembalian ? new Date(item.tgl_pengembalian).toLocaleDateString('id-ID') : '-',
      'Status': item.status || '-',
    ...
    XLSX.writeFile(wb, `peminjaman-${new Date().getTime()}.xlsx`);
  };
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1">Approve atau tolak permintaan peminjaman</p>
            </div>
